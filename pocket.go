...
const (
	...
	authorizeUrl = "https://getpocket.com/auth/authorize?request_token=%sredirect_uri=%s"
...
func (i AddInput) validate() error {
	if i.URL == "" {
		return errors.New("requested URL values is empty")
	}
...
// Client is a getpocker API client
type Client struct {
	client      *http.Client
	consumerKey string
}
...
func (c *Client) doHTTP(ctx context.Context, endpoint string, body interface{}) (url.Values, error) {
	...
	if err != nil {
		return url.Values{}, errors.New("failed to create context request")
	}
